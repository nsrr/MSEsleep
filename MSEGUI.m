function varargout = MSEGUI(varargin)
% MSEGUI MATLAB code for MSEGUI.fig
%      MSEGUI, by itself, creates a new MSEGUI or raises the existing
%      singleton*.
%
%      H = MSEGUI returns the handle to a new MSEGUI or the handle to
%      the existing singleton*.
%
%      MSEGUI('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in MSEGUI.M with the given input arguments.
%
%      MSEGUI('Property','Value',...) creates a new MSEGUI or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before MSEGUI_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to MSEGUI_OpeningFcn via varargin.
%
...
%   https://github.com/DennisDean/MSEGUI/blob/master/README.md
%
% Related websites:
%
...
% Edit the above text to modify the response to help MSEGUI

% Last Modified by GUIDE v2.5 08-Mar-2017 15:51:27
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @MSEGUI_OpeningFcn, ...
                   'gui_OutputFcn',  @MSEGUI_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before MSEGUI is made visible.
function MSEGUI_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to MSEGUI (see VARARGIN)

% Choose default command line output for MSEGUI
...
set(handles.e_analysis_parameters_analysis_signals, 'String','{''C3''}');
set(handles.edit15, 'String','edf.xml');
...
% UIWAIT makes MSEGUI wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = MSEGUI_OutputFcn(hObject, eventdata, handles) 
...
xmlSuffix = get(handles.edit15, 'String');
complexityall(dataFolder,resultFolder, ...
    analysisSignals, referenceSignals, referenceMethod, ...
    ecg_name, startFile,xmlSuffix);
